...
#        self.send_debug_trap()
    ...
    def MAVLinkHeaderTargets(self):
        """Header targets override payload targets, including on heartbeats."""
        ...
        try:
            for source in (42, 70000, 0xFFFFFFFF):
                ...
                # This request's payload names a different vehicle. Only
                # the explicit header target makes it reach our autopilot.
                request = mav2.mav.param_request_read_encode(99, 1, b"MAV_SYSID", -1)
                request.set_target(self.sysid_thismav(), 1)
                ...
                if reply is None or reply.param_id != 'MAV_SYSID':
                    raise NotAchievedException("Header target did not override payload")

                request = mav2.mav.param_request_read_encode(self.sysid_thismav(), 1, b"FRAME_CLASS", -1)
                request.set_target(99, 1)
                mav2.mav.send(request)
                if mav2.recv_match(type='PARAM_VALUE', condition='PARAM_VALUE.param_id == \"FRAME_CLASS\"',
                                   blocking=True, timeout=1) is not None:
                    raise NotAchievedException("Processed request for a different header target")

                # Our main connection is a learned destination. An explicitly
                # targeted heartbeat must not escape to an unrelated route.
                for target in (self.mav.source_system, self.mav.source_system ^ 0x100):
                    heartbeat = mav2.mav.heartbeat_encode(mavutil.mavlink.MAV_TYPE_GCS, 0, 0, 123, 0)
                    heartbeat.set_target(target, self.mav.source_component)
                    wire = heartbeat.pack(mav2.mav)
                    mav2.write(wire)
                    received = self.mav.recv_match(
                        type='HEARTBEAT', condition='HEARTBEAT.custom_mode == 123',
                        blocking=True, timeout=2)
                    if target == self.mav.source_system:
                        if received is None or bytes(received.get_msgbuf()) != bytes(wire):
                            raise NotAchievedException("Targeted heartbeat was not forwarded intact")
                    elif received is not None:
                        raise NotAchievedException("Targeted heartbeat leaked to an unrelated route")
        finally:
            mav2.close()
        ...
        try:
            for sysid in (0x01000001, 0x7FFFFFFF, 0x80000000, 0xFFFFFFFE, 0xFFFFFFFF):
                ...
                if any(actual[name] != value for name, value in expected.items()):
                    raise NotAchievedException("Unsigned system IDs changed across reboot")
