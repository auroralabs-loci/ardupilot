...
void AP_Mount_MAVLink::send_gimbal_device_attitude_status(mavlink_channel_t chan)
{
    auto *out_link = gcs().chan(chan);
    if (_link == nullptr || out_link == nullptr || _last_attitude_status_ms == 0 ||
        AP_HAL::millis() - _last_attitude_status_ms > 1000) {
        return;
    }
    WITH_SEMAPHORE(comm_chan_lock(chan));
    if (out_link->sending_mavlink1() || !HAVE_PAYLOAD_SPACE(chan, GIMBAL_DEVICE_ATTITUDE_STATUS)) {
        return;
    }
    mavlink_gimbal_device_attitude_status_t status = _gimbal_device_attitude_status;
    status.target_system = 0;
    status.target_component = 0;
    status.gimbal_device_id = 0;
    mavlink_message_t msg;
    mavlink_msg_gimbal_device_attitude_status_encode_chan(_sysid, _compid, chan, &msg, &status);
...
void AP_Mount_MAVLink::find_gimbal()
{
    ...
    if (_link == nullptr) {
        ...
        const uint8_t compid = get_mavlink_device_id();
