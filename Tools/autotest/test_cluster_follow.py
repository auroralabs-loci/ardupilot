...
SHM_HEADER_LEN = 16                       # magic, version, _reserved[2]
SHM_SLOT_LEN = 8 + 4 + 4 + 40 + 4096      # sim_time, pid, pad, mutex, payload
SHM_MAGIC = 0x4150534F
...
class ClusterView(object):
    ...
    def members(self):
        ...
        for i in self.instances:
            sim_time, pid = struct.unpack_from(
                "<qi", self.mm, SHM_HEADER_LEN + i * SHM_SLOT_LEN)
            if pid > 0:
                out[i] = sim_time
        return out
...
class Vehicle(object):
    ...
    def wait_ready_to_arm(self, sim_limit=300):
...
class ClusterFollowTest(object):
    ...
    def run(self):
        ...
        # Slow the plane down and speed the copters up, or the copters
        # simply cannot keep station with a fixed-wing aircraft.
        progress("configuring plane")
        self.plane.set_params({
            "AIRSPEED_CRUISE": 12,   # m/s - slow enough for copters to hold
            "AIRSPEED_MIN": 9,
            "AIRSPEED_MAX": 16,
            "WP_LOITER_RAD": 250,    # wide circle keeps the turn rate gentle
            "TKOFF_ALT": 100,
        })

        progress("configuring copters")
        for c in self.copters:
            ofs = self.OFFSETS[c.name]
            c.set_params({
                "FOLL_ENABLE": 1,
                "FOLL_SYSID": self.plane.sysid,
                "FOLL_OFS_TYPE": 1,      # relative to lead vehicle heading
                "FOLL_OFS_X": ofs[0],
                "FOLL_OFS_Y": ofs[1],
                "FOLL_OFS_Z": ofs[2],
                "FOLL_ALT_TYPE": 0,      # absolute, match the plane's frame
                "FOLL_DIST_MAX": 1000,   # metres; this is the param maximum
                "FOLL_TIMEOUT": FOLL_TIMEOUT_S,
                "FOLL_YAW_BEHAVE": 1,
                # AC_WPNav is registered under the "WP_" prefix in Copter
                # (GOBJECTPTR(wp_nav, "WP_", ...)), and these are m/s, not
                # the older cm/s WPNAV_SPEED/ACCEL. 20 m/s is the parameter
                # maximum, and is what lets a copter hold station on a
                # 12 m/s plane.
                "WP_SPD": 20,
                "WP_SPD_UP": 5,
                "WP_SPD_DN": 5,
        ...
        self.pump(lambda s: self.plane.rel_alt() > 80,
                  sim_limit=300, desc="plane-takeoff", bridge=False)
        ...
        self.pump(lambda s: all(c.rel_alt() > 50 for c in self.copters),
                  sim_limit=300, desc="copter-takeoff")
...
def main():
    ...
    parser.add_option("", "--copter-binary",
                      default="build/sitl/bin/arducopter",
                      help="path to the ArduCopter SITL binary")
    parser.add_option("", "--plane-binary",
                      default="build/sitl/bin/arduplane",
                      help="path to the ArduPlane SITL binary")
    parser.add_option("", "--cluster", type="int", default=91,
                      help="cluster id to use")
    parser.add_option("", "--speedup", type="int", default=50,
                      help="requested SITL speedup, applied to every vehicle "
                           "in the cluster (default 100). The achieved rate "
                           "is measured and reported as the test runs.")
    parser.add_option("", "--loop-dt", type="float", default=0.01,
                      help="bridge/telemetry loop period, WALL seconds. "
                           "speedup * loop-dt is the sim-time gap between "
                           "position updates the copters see; it must stay "
                           "well under FOLL_TIMEOUT (%.0fs)." % FOLL_TIMEOUT_S)
    parser.add_option("", "--follow-time", type="float", default=600.0,
                      help="SIM seconds to hold formation for")
    parser.add_option("", "--wall-limit", type="float", default=900.0,
                      help="wall-clock backstop per phase in seconds; only "
                           "stops a wedged run, it is not the real timeout")
    parser.add_option("", "--report-interval", type="float", default=5.0,
                      help="WALL seconds between progress lines")
    parser.add_option("", "--hold-distance", type="float", default=250.0,
                      help="metres of formation error that still counts as "
                           "holding station (distance from the copter to the "
                           "offset position it is commanded to hold)")
    parser.add_option("", "--min-hold-time", type="float", default=120.0,
                      help="SIM seconds each copter must hold formation for "
                           "WITHOUT a break. The plane laps its loiter in "
                           "roughly 130s, so this cannot be satisfied by a "
                           "copter that is merely being orbited past.")
    parser.add_option("", "--max-skew-us", type="int", default=None,
                      help="maximum tolerated cluster clock skew in sim "
                           "microseconds. The barrier's window is fixed in "
                           "WALL time and scaled by the speedup, so the "
                           "default here scales the same way: 10000 * speedup")
