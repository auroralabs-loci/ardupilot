...
In the front view, I2C is the lowest connector on the left edge. In the back
view, CAM and DJI are on the left edge and CAN is on the right edge. The BOOT
button is the tactile switch at the bottom center of the back view.
...
| SERIAL4 | UART4 | TELEM2 | MAVLink2 |
| SERIAL5 | UART5 | TELEM3 | MAVLink2 |
...
| TELEM3 | 1 GND, 2 5V, 3 TX5, 4 RX5 |
| TELEM2 | 1 GND, 2 5V, 3 TX4, 4 RX4 |
...
The STM32H7 USART provides hardware signal inversion. An inverted SBUS receiver
can be connected directly to USART6 RX by setting `SERIAL6_OPTIONS` to `1`
(InvertRX); no external inverter is required.
...
to SmartAudio (`37`) or IRC Tramp (`44`).
...
`arducopter_with_bl.hex` image with STM32CubeProgrammer. Subsequent updates can
be installed using the `.apj` file from an ArduPilot ground station.
