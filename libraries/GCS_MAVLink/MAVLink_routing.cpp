                                        ...
                                        const mavlink_message_t &msg)
{
    ...
    // Messages excluded from forwarding must still honor explicit targets.
    const bool local_target = !targetted ||
        ((msg.target_sysid == 0 || msg.target_sysid == mavlink_system.sysid) &&
         (msg.target_compid == 0 || msg.target_compid == mavlink_system.compid));
    ...
    if (msg.msgid == MAVLINK_MSG_ID_HEARTBEAT && !targetted) {
        // untargeted heartbeat needs special handling
