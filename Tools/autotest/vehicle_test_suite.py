...
#        frsky.verbose = True
    ...
    def MAVFTPListDirectoryWithTimeTabInName(self):
        ...
        try:
            # a request can only read past the skip entry if something is
            # listed after it.  where readdir puts a name depends on the name
            # and the filesystem, so try names until one is not listed last
            for attempt in range(10):
                tab_name = f"tab\there_{attempt}.txt"
