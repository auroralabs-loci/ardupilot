...
/*
  @MAV_LOG is only worth its flash where the logs are actually written to a
  filesystem.  the test lives here rather than in AP_Filesystem_config.h
  because AP_Logger_config.h includes that header, so the macro is not yet
  defined while it is being read and would silently read as zero
 */
...
/*
  the rewritten path has to hold this board's log directory, a separator and
  the longest path an FTP listing stats: the longest path a request can
  carry, a separator and a 255 byte name.  a board whose log directory is
  long enough to break that would otherwise lose files from a listing with
  no sign of why
 */
...
#endif
#if AP_FILESYSTEM_MAVLOG_ROW_ENABLED
    // an alias rather than a filesystem of its own: the local filesystem
    // serves it, under the directory this board logs to
...
const AP_Filesystem::Backend &AP_Filesystem::backend_by_path(const char *&path) const
{
    ...
    for (uint8_t i=1; i<NUM_BACKENDS; i++) {
        ...
        // the prefix has to be the whole of the first component, or
        // "@MAV_LOG_backup" would be served as "@MAV_LOG" plus "_backup"
...
/*
  resolve a path to the backend which serves it, rewriting the path when the
  prefix it matched is an alias
 */
...
#endif
{
    ...
    if (buffer == Buffer::OWN) {
        ...
        // see the warning on alias_sem: this is held until the destructor
    ...
    // the root may already end in a separator, and another would name a
    // different place.  an empty path is the aliased directory itself
    ...
    if (uint32_t(hal.util->snprintf(rewritten, AP_FILESYSTEM_ALIAS_PATH_MAX, "%s%s%s", dir, sep, _path)) >= AP_FILESYSTEM_ALIAS_PATH_MAX) {
        // no room to say what was asked for.  there is no path we could
        // hand on which is certain to name nothing - the part after the
        // prefix and the whole of it as it arrived can each name a file
        // which exists - so refuse instead
        _path = nullptr;
        errno = ENAMETOOLONG;
        return;
    }
...
int AP_Filesystem::rename(const char *oldpath, const char *newpath)
{
    ...
    // this is the second path alive at once, so it cannot share the
    // buffer the first may be using
...
struct dirent *AP_Filesystem::readdir(DirHandle *dirp)
{
    ...
    for (; ret == nullptr && virtual_dirent.backend_ofs < ARRAY_SIZE(AP_Filesystem::backends); virtual_dirent.backend_ofs++) {
        ...
        // an alias is served by a filesystem it shares with other paths, so
        // it has to be probed at its own root or it would be offered even
        // when the directory it stands for is not there
